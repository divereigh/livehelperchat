...
<?php elseif (!$sysonly) : ?>
<div class="message-row<?php echo $msg['user_id'] == 0 ? ' response operator-changes' : ' message-admin operator-changes' ?>" data-op-id="<?php echo $msg['user_id']?>" id="msg-<?php echo $msg['id']?>">
	<div class="msg-date"><?php echo erLhcoreClassChat::formatDate($msg['time']);?></div><span class="usr-tit<?php echo $msg['user_id'] == 0 ? ' vis-tit' : ' op-tit'?>"><?php echo $msg['user_id'] == 0 ? '<i class="material-icons chat-operators mi-fs15 mr-0">face</i>'.htmlspecialchars($chat->nick) : '<i class="material-icons chat-operators mi-fs15 mr-0">account_box</i>'.htmlspecialchars($msg['name_support']) ?></span><?php echo erLhcoreClassBBCode::make_clickable(htmlspecialchars($msg['msg']))?></div>
<?php endif;?>

	<?php endforeach; ?>
<?php if ($sysonly && $lastOperatorChanged) : ?>
<div class="message-row system-response">
        <i><span class="usr-tit sys-tit"><?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/syncadmin','System assistant')?></span>This chat has now been transferred (you should now close the chat).</i>
</div>
<?php endif;?>

<?php else : ?>
	<?php foreach ($messages as $msg ) : ?>
<div class="message-row<?php echo $msg['user_id'] == 0 ? ' response operator-changes' : ' message-admin operator-changes'?>" id="msg-<?php echo $msg['id']?>">
