...
//browser notifcation popup function
this.showPopupNotification = function(message) {
  //if browser supports notifications
  if ("Notification" in window) {
	var options = {
      body: message,
      icon: "/lhc_web/design/defaulttheme/images/general/logo.png"
	}
	  //check if user has allowed for notifications on the browser
	  if (Notification.permission === "granted") {
		//show popup notification
		var notification = new Notification("ABA Chat",options);
	  }

	  //if user hasn't allowed notifications, ask for permission first
	  else if (Notification.permission !== 'denied') {
		Notification.requestPermission(function (permission) {
		  //If user accepts, show popup
	...
	this.loadChatList = function(filter){
		...
		$http.get(WWW_DIR_JAVASCRIPT + 'chat/syncadmininterface' + filter).success(function(data) {
			 if (typeof data.error_url !== 'undefined') {
				...
				//check if there is a pending alert to let user know a visitor has been waiting a while
				//these occur when ever the visitor gets an auto response from the system
				for (var i = 0; i < data.result.pending_chats.list.length; i++) {
					//alert(data.result.pending_chats.list[i].alert_user_wait_time_pending);
					if (data.result.pending_chats.list[i].alert_user_wait_time_pending) {
						this.showPopupNotification("Visitor "+data.result.pending_chats.list[i].nick+" has waited "+data.result.pending_chats.list[i].wait_time_pending);
					}
				}
			 }			 
		}).error(function(){
					...
					if ( item.last_id_identifier ) {
	                    if (lhinst.trackLastIDS[item.last_id_identifier] == undefined ) {
	                    	...
	                    	if (lastNotifiedId != lhinst.trackLastIDS[item.last_id_identifier]) {
								...
								//new chat audio won't always play, so force audio to play now.
								//lhinst.playNewChatAudio();
								var audio = new Audio();
...
//		window.open(WWW_DIR_JAVASCRIPT + 'chat/startchatwithoperator/'+user_id,'operatorchatwindow-'+user_id,'menubar=1,resizable=1,width=780,height=450');
	...
	this.setOtherOperatorOnlineStatus = function(user_id,online_status) {	
		var inst = this;
		$.postJSON(WWW_DIR_JAVASCRIPT + 'user/set_other_operator_online_status/'+user_id+"/"+online_status,{'direct':1}, function(data){
			if (data.error == 'false') {
	...
	this.logoutOtherOperator = function(user_id) {	
		var inst = this;
		$.postJSON(WWW_DIR_JAVASCRIPT + 'user/logout_other_operator/'+user_id,{'direct':1}, function(data){
			if (data.error == 'false') {
				$scope.loadChatList();
			};
		});
